...
for(i in 1:length(chosen_vec)) {  # FUNKCNI; 'chosen_vec' = CZSK × 'wsnames' = CZ
  a <- read.table(paste0(path_met, chosen_vec[i])) # reads the DLY file
  ...
  af$source <- paste0(chosen_vec[i]) # adds column with the DLY name
  ...
  write.table(af, file = paste0(path_out, crop, "/phusum/", chosen_vec[i], ".csv"), sep = ",", row.names = F)
} 
...
# phu_cal0 <- merge(eea10_simu, phusumA, by.x = "GRID_NO", by.y = "source", all.y = T) # 1427 grid * 15 cultivars (BARL) = 21405
...
phu_cal1 <- phusumA %>% 
  ...
  mutate(cntry = case_when(GRID_NO %in% 1:891 ~ "CZ",
...
opsc3 <- opsc2 %>% # 8640
  ...
  select(cntry, runid, scenario, GRID_NO,	CROPID,	CROP,	CropSpec,	OPERATION,	LUN, Year, OPMONTH,	OPDAY,
